...
# .chop is like .pop but for strings
  def test_chop
...
# .strip removes empty space on the edges. .rstrip = right strip, .lstrip = left strip.
  def test_strip
    assert_equal "to the    moon", "   \n  \tto the    moon\n\n\t    ".strip
    assert_equal "   \n  \tto the    moon", "   \n  \tto the    moon\n\n\t    ".rstrip
    assert_equal "to the    moon\n\n\t    ", "   \n  \tto the    moon\n\n\t    ".lstrip
  end
# .next changes the last character to the next in line. This is weird.
  def test_next
...
# replaces characters matching the first argument for the second argument
  def test_tr
...
# Does teh same thing as tr, but only changes the first matching character
  def test_sub
...
# Like tr but can handle more complex patterns
  def test_gsub
